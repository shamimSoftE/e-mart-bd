...
! function(a, b) {
    ...
    function g(b, c) {
        ...
        var e = b.find(".section > *");
        e.removeData("loaded").removeData("mode").removeData("url"), e.removeAttr("id").removeAttr("role").removeAttr("tabindex").removeAttr("class").removeAttr("style")._removeAria("labelledby")._removeAria("hidden"), b.find(".section > [data-mode='async'],.section > [data-mode='iframe']").empty();
        var f = a('<{0} class="{1}"></{0}>'.format(b.get(0).tagName, b.attr("class"))),
            g = b._id();
        return null != g && "" !== g && f._id(g), f.html(b.find(".section").html()), b.after(f), b.remove(), f
    }
    ...
    function A(b, c, d) {
        if (d.stepCount > 0) {
            ...
            if (!c.enableContentCache || !f.contentLoaded) switch (r($, f.contentMode)) {
                case $.iframe:
                    b.find(".section > .body").eq(d.currentIndex).empty().html('<iframe src="' + f.contentUrl + '" frameborder="0" scrolling="no" />').data("loaded", "1");
                    break;
                case $.async:
    ...
    function J(b, c, d) {
        var e = '<{0} class="{1}">{2}</{0}>',
            ...
            h = a(e.format(c.contentContainerTag, "section " + c.clearFixCssClass, b.html())),
